# -*- coding: utf-8 -*-
"""24W_CS188_Assignment3.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1ONbP_GsSLyQOTGlR6pHtHOMJ_MuuIWur
...
def tokenize_image(img, patch_size=16, stride=16):
    ...
    for i in range(0, H, stride):
        for j in range(0, W, stride):
            ...
            patch = patch.permute(1, 2, 0).flatten()
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        scores = scores / torch.sqrt(torch.tensor(self.dim_head, dtype=torch.float32))
        attn_weights = self.softmax(scores)
        attn_weights = self.dropout(attn_weights)
        attn_weights = attn_weights@v
        out = rearrange(attn_weights, 'b h n d -> b n (h d)')
...
class ViT(nn.Module):
    ...
    def __init__(self, *,
        ...
        self.transformers = nn.ModuleList([Transformer(dim, heads, dim_head, mlp_dim, dropout) for _ in range(depth)])
...
Your Answer: ResNet quickly learns from small image details, while ViT needs more time to understand the bigger picture by considering patches around each pixel, which can slow down its training when time is limited.
...
class OxfordIIITPetSegmentation(Dataset):
    ...
    def __init__(
        ...
        with open( self.anns_folder / txt_file, 'r') as file:
          for line in file:
            columns = line.split()
            image_name = columns[0]
            image_path = self.images_folder / f"{image_name}.jpg"
            segmap_path = self.segmaps_folder / f"{image_name}.png"
    ...
    def load_segmentation_map_tensor(self, idx):
        ...
        transform_to_tensor = transforms.Compose([
            transforms.ToTensor(),
            transforms.Lambda(lambda x: ((x * 255)-1).round().clamp(0, 2).to(torch.long))
        ])
        segmap = transform_to_tensor(segmap)
...
class MySegmentationDecoder(nn.Module):
    def __init__(self,
        ...
        self.conv2 = nn.Conv2d(dim, num_classes, kernel_size=1, stride=1, padding=0, bias=False)
    ...
    def forward(self, x):
        ...
        x = self.conv2(x)
        out = nn.functional.interpolate(x, size=(self.image_size, self.image_size), mode = "nearest")
...
class MyVITSegmentationModel(nn.Module):
  def __init__(self,
    ...
    self.decoder=MySegmentationDecoder(image_size=image_size,
                 patch_size=patch_size,
                 num_classes=num_classes,
                 dim=dim)
...
def mean_iou(logits, target):
  ...
  for label in range(num_classes):
    intersection = ((preds == label) * (target == label)).sum().item()
    union = ((preds == label) + (target == label)).sum().item()
    iou = (intersection + eps) / (union + eps)
    mean_iou += iou

  out = mean_iou/num_classes
...
def dice_coeff(logits, target):
  ...
  for label in range(num_classes):
    intersection = ((preds == label).float() * (target == label).float()).sum().item()
    pred_sum = torch.sum(preds == label).float()
    target_sum = torch.sum(target == label).float()
    dice_coefficient = (2.0 * intersection + eps) / (pred_sum + target_sum + eps)
    mean_dice += dice_coefficient.mean().item()

  out = mean_dice / num_classes
...
Your Answer: IoU measures how much predicted and ground truth masks overlap, while DICE score assesses overall agreement by considering both false positives and false negatives. IoU focuses on overlap and DICE considers overall agreement, making them valuable for comprehensive evaluation.
